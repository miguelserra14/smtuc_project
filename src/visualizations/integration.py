...
def _build_direction_figures(
...
) -> tuple[object, object, dict | None, pd.DataFrame]:
    """Constrói as 2 figuras (espera, equidade) para UM sentido de transbordo, com o horário
    "atual" sempre visível mais uma trace extra (visible=False) por cada variante de `variants` -
    ex.: "otimizado" (ver `config.LINE_54_OPTIMIZED_PHASE_SHIFT_MIN`) e "melhorado" (ver
    `config.LINE_54_MELHORADO_PHASE_SHIFT_MIN`) - selecionável por um botão Atual/Otimizado/
    ...
    As 2 figuras usam SEMPRE a mesma escala (eixo Y da espera, cor da equidade) nas variantes
    atual/otimizado/melhorado - sem isto o autorange do Plotly recalcula a escala consoante a
    variante visível e mascara a melhoria real (o eixo encolhe junto com os valores e as barras
    parecem na mesma). O gráfico de espera usa ainda um eixo X partilhado (união cronológica de
    todos os horários-gatilho de todas as variantes), necessário porque "melhorado" tem o dobro
    das chegadas no sentido inverso - um índice posicional próprio a cada variante desalinhava ou
    transbordava os rótulos do eixo."""
    ...
    # 1) Wait-time bar plot with point overlays for special cases
    ...
    # 2) Period equity heatmap (cobertura <=10, perdas >15, espera média)
    ...
    # todas as traces de cada figura já existem. Mutação feita sobre `fig_heat.data` (não sobre
    # os objetos `go.Heatmap` originais) porque `Figure.add_trace` guarda cópias internas - mutar
    # o objeto que foi passado para `add_trace` não afeta o que fica na figura.
    wait_maxes = [
    ...
    return fig_wait, fig_heat, variants_payload, waits_df
...
def create_combined_integration_dashboard(
    ...
    """Create a single HTML page with waits and equity in one document.
    ...
    `_build_direction_figures` (chaves `modes`, `labels`, `notes`, `waits`, `equity`). Sem isto, o
    grupo de botões Atual/Otimizado/Melhorado não é mostrado.

    `reverse` (opcional) descreve o sentido inverso (bus->metro, ver
    `build_reverse_stop_vs_metro_table`) a mostrar como 2 painéis extra na mesma página - deve
    ter as chaves `waits_fig`/`equity_fig` e opcionalmente `variants` (mesmo formato do parâmetro
    acima). Sem isto, a página só mostra o sentido metro->bus.
    """
    if pio is None:
...
def generate_connection_visualizations(
    ...
    fig_wait, fig_heat, variants_payload, waits_df = _build_direction_figures(
        schedule_df,
        [
    ...
    if reverse_metro_direction_id is not None:
        ...
        fig_wait_rev, fig_heat_rev, variants_payload_rev, waits_df_rev = _build_direction_figures(
            schedule_df_rev,
            [
